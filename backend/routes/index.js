...
router.get('/patch/', (req, res, next) => {
  res.status(405).json({
    Aviso: "Error.Necesitas especificar el 'id' del Sistema Operativo que deseas modificar "
  })
});
...
router.patch('/update/:id', async (req, res, next) => {
  const {
   ...
   Sistema.updateOne({
    ...
    if (err) {
      res.status(404).json({
        Aviso: "Error. No se ha podido modificar el documento"
      });
    } else {
      if (info == null) {
        res.status(404).json({
          Aviso: "Error. No existe el 'id' que deseas modificar"
        });
      } else {
        res.status(200).json({
          Aviso: "Se ha modificado el S.O. de forma eficaz"
        });
      }
    }
  });
